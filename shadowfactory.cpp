...
QImage ShadowFactory::createInsideShadow(const QSize &baseSize, qreal radius, const QColor &shadowColor, const QColor &fillColor, qreal devicePixelRatio)
{
    if(baseSize.isValid())
    {
        const int _radius = qCeil(radius);
        const qreal ratio = devicePixelRatio;
        const int _border = _radius * 2;
        QRect rect(_border, _border, baseSize.width(), baseSize.height());
        QSize imageSize = QSize(rect.right() + _border + 1, rect.bottom() + _border + 1) * ratio;
        ...
        if(radius > 0)
        {
            ...
            blurPainter.fillRect(target.rect(), shadowColor);
            blurPainter.end();
        }
        target.setDevicePixelRatio(ratio);

        QImage result(baseSize * ratio, QImage::Format_ARGB32_Premultiplied);
        result.fill(fillColor);
        painter.begin(&result);
        painter.drawImage(QPoint(0, 0), target, rect);
        painter.end();
        return result;
    }
    return QImage();
